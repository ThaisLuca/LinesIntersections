 ...
 */
bool AVL::add(int data, int lineIndex){
	bool was_added = false;
	
	if (root == NULL){
		root = new Node;
		root->data = data;
		root->lineIndex = lineIndex;
		was_added = true;
	} else {
		if (contains(data, root) == NULL){
			root = recursiveAdd(data, lineIndex, root);
			was_added = true;
		} else {
			was_added = false;
		}
...
Node* AVL::recursiveAdd(int data, int lineIndex, Node* current_search){
	if (current_search == NULL){
		Node* location_added = new Node;
		location_added->data = data;
		location_added->lineIndex = lineIndex;
		return balance(location_added);
	}
	
	if (data > current_search->data){
		current_search->right_child = recursiveAdd(data, lineIndex, current_search->right_child);
	} else {
		current_search->left_child = recursiveAdd(data, lineIndex, current_search->left_child);
	}
...
int AVL::getNext(int data){
	Node* node = contains(data, getRootNode());

	if(node == NULL || node->right_child == NULL){
		return -1;
	}
	return node->right_child->lineIndex;
}

int AVL::getPrev(int data){
	Node* node = contains(data, getRootNode());

	if(node == NULL || node->left_child == NULL){
		return -1;
	}
	return node->left_child->lineIndex;
}
